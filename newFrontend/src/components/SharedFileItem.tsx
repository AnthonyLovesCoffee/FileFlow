...
interface SharedFileItemProps {
  file: FileShare;
  type: 'shared-by-me' | 'shared-with-me';
  onShare?: (fileId: number, fileName: string) => void;
  onRevoke?: (fileId: number, username: string) => void;
  onDownload?: (fileId: number, fileName: string) => void;
  isDownloading?: boolean;
  downloadProgress?: number;
}

export const SharedFileItem: React.FC<SharedFileItemProps> = ({
  file,
  type,
  onShare,
  onRevoke,
  ...
  console.log('SharedFileItem render:', {
    fileId: file.file.id,
    isDownloading,
    downloadProgress,
    type
    ...
    <div className="p-4 border rounded-lg hover:bg-gray-50">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <p className="font-medium">{file.file.fileName}</p>
            <div className="grid grid-cols-2 gap-4 mt-2 text-sm text-gray-500">
              <p>Size: {formatFileSize(file.file.fileSize)}</p>
              <p>{type === 'shared-by-me'
                  ? `Shared with: ${file.sharedWithUsername}`
                  : `Shared by: ${file.sharedByUsername}`}
              </p>
              <p>Shared on: {new Date(file.sharedDate).toLocaleDateString()}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex space-x-2">
          {type === 'shared-by-me' ? (
            <>
              <button
                onClick={() => onShare?.(file.file.id, file.file.fileName)}
              ...
              <button
                onClick={() => onRevoke?.(file.file.id, file.sharedWithUsername)}
            ...
            <button
              onClick={() => onDownload?.(file.file.id, file.file.fileName)}
